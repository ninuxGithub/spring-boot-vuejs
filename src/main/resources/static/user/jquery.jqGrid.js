	...
	from : function(source){
		...
		this._compare=function(a,b,d){
			...
			if (!_usecase && typeof a !== "number" && typeof b !== "number")
			{
			    ...
			    //console.dir("a"+a.charAt(0) +"--"+a.charAt(0).charCodeAt()+" b:"+b.charAt(0)+"--"+b.charAt(0).charCodeAt())
			    var regcl= /^[A-Za-z]+$/;
			    var reg = new RegExp("[\\u4E00-\\u9FFF]+", "g");
			    if (reg.test(a) || regcl.test(a))
			    {
			        //alert("包含汉字！");
			        /*中文比较*/
			        if (typeof a === 'string' && typeof b === 'string')
			        {
			        	var _compareValue=0;
			        	if(isStockType){
			        		_compareValue = typeMap[a].charCodeAt()-(typeMap[b].charCodeAt());
			        	}else{
			        		_compareValue = a.localeCompare(b);
			        	}
			            if (_compareValue > 0)
			            {
			                return d;
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		modifyData = function(data, st, xhr){
			if ($.isFunction(ts.p.beforeProcessing)) {
				return ts.p.beforeProcessing.call(ts, data, st, xhr);
			}
			return data;
		},
...
//						pot = [($.jgrid.getRegional(ts,"defaults.pgfirst",ts.p.pgfirst) || "首页"),
//								($.jgrid.getRegional(ts,"defaults.pgprev",ts.p.pgprev) || "上一页"),
//								($.jgrid.getRegional(ts,"defaults.pgnext",ts.p.pgnext) || "下一页"),
//								($.jgrid.getRegional(ts,"defaults.pglast",ts.p.pglast) || "尾页")];
						pot = ["首页","上一页","下一页","尾页"];
	...
	setGridHeight : function (nh) {
		return this.each(function (){
			...
			if($t.p.frozenColumns === true){
				...
				$('#'+$.jgrid.jqID($t.p.id)+"_frozen").parent().height(bDiv.height() - 11);
